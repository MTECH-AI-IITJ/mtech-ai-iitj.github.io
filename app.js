...
function initializeStudyPlanner() {
  ...
  if (!prevWeekBtn || !nextWeekBtn || !currentWeekLabel) return;
...
function renderScheduleContainer(containerId, courses, isToday = false) {
  ...
  container.innerHTML = courses.map(course => {
    ...
    let statusBadge = '';
    if (isToday && course.startDate && course.startDate.toLowerCase().includes('first class')) {
    ...
    return `
      <div class="schedule-item">
        ...
        <div class="course-info">
          ...
          <div class="instructor">${Array.isArray(course.instructors) ? course.instructors.join(', ') : course.instructors}</div>
          ${meetingInfo}
          ${statusBadge}
        </div>
      </div>
...
function validateSchedule() {
  const weekendCourses = Object.values(courseData).filter(course => 
    course.days && (
      (Array.isArray(course.days) && (course.days.includes('Saturday') || course.days.includes('Sunday'))) ||
